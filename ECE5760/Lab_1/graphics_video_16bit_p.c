...
void *edit_thread(void *args){
	while(1){
		...
		//printf("this is edit_thread");
		if(edit == 1){
			printf("edit in thread == %d", edit);
		}
		//printf("edit in thread == %d", edit);
		if(edit == 1){
...
//	temp_x2 = (-1048576 / 1048576.0)*3 + 150;
	temp_y2 = (104857 / 1048576.0)*3 + 500;
	temp_z2 = (26214400 / 1048576.0)*3 + 150;

		// the thread identifiers
		pthread_t thread_main, reset_thread_main, thread_up, thread_down, thread_pause, thread_edit;
		//pthread_t thread_input;
		// now the threads
		pthread_create(&thread_main, NULL, thread1, NULL);
